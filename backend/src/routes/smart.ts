...
function avgPredictions(predictions: unknown): number | null {
  if (predictions == null || !Array.isArray(predictions)) return null;
  const nums = predictions.filter((x): x is number => typeof x === "number");
...
router.get("/risk", async (_req, res) => {
  ...
  for (const it of items) {
    ...
    if (f) {
      const preds = f.predictions as unknown;
...
router.get("/warehouse", async (_req, res) => {
  ...
  if (primary && secondary && pAct > sAct * 2 && pAct > 0) {
    hints.push(
      `${primary.code} shows the highest pick activity — keep fastest movers and pick paths consolidated here.`
    );
  }
...
router.post("/reorder/generate", async (_req, res) => {
  const items = await prisma.item.findMany();
  const upserted: { id: string; sku: string; suggestedQty: number }[] = [];

  for (const it of items) {
    const q = Number(it.currentQuantity);
    const rl = it.reorderLevel;
    if (rl <= 0 || q > rl) continue;
