...
export async function toolSearchItems(args) {
...
export async function toolGetItemBySku(args) {
    const sku = typeof args.sku === "string" ? args.sku.trim() : "";
    if (!sku)
        return { found: false, message: "sku required" };
...
export async function toolGetCatalogStats(_args) {
...
export async function toolListCategories(_args) {
...
export async function dispatchInventoryTool(name, args) {
    switch (name) {
        case "list_low_stock":
