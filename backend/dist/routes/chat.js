...
async function ruleBasedReply(textRaw) {
    ...
    let reply = "Try asking about low stock, how many items, or a SKU. Set OPENAI_API_KEY on the server for a full AI assistant with live tool lookups.";
    if (text.includes("low stock") || text.includes("reorder")) {
        ...
        if (skuMatch) {
            ...
            if (found) {
                reply = `${found.name} (${found.sku}): quantity ${Number(found.currentQuantity)} ${found.unit}. Reorder at ${found.reorderLevel}.`;
            }
        }
    }
    return reply;
