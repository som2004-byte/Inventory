...
async function dailyNetSeries(itemId: string, daysBack: number): Promise<number[]> {
...
function movingAverageLocal(history: number[], horizon: number): { predictions: number[]; model: string } {
  if (history.length === 0) {
